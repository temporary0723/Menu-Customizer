...
function setupDragAndDrop() {
    ...
    currentModal.on('touchstart', '.menu-customizer-item', function(e) {
        ...
        longPressTimer = setTimeout(() => {
            ...
            draggedItem = $item;
            draggedFrom = $item.parent();
            
            $item.addClass('dragging');
            
            // 플레이스홀더 생성
            touchPlaceholder = $('<div class="menu-customizer-item-placeholder"></div>');
            touchPlaceholder.css({
                height: $item.outerHeight() + 'px',
                background: 'rgba(124, 124, 255, 0.2)',
                border: '2px dashed rgba(124, 124, 255, 0.5)',
                borderRadius: '8px',
                margin: '4px 0'
            });
            ...
            toastr.info('드래그 모드 활성화', '', { timeOut: 1000 });
        }, 300);
    });
    
    // 터치 이동
    currentModal.on('touchmove', '.menu-customizer-item, .menu-customizer-list, .menu-customizer-category-items', function(e) {
        if (!isDragging || !touchDraggedItem) return;
        ...
        const $targetItem = $elemBelow.closest('.menu-customizer-item');
        const $targetCategory = $elemBelow.closest('.menu-customizer-category-items');
        const $targetList = $elemBelow.closest('.menu-customizer-list');
        
        // 기존 drag-over 제거
        currentModal.find('.drag-over').removeClass('drag-over');
        
        // 플레이스홀더 위치 업데이트
        if ($targetItem.length && !$targetItem.is(touchDraggedItem)) {
            $targetItem.addClass('drag-over');
            touchPlaceholder.detach();
            $targetItem.before(touchPlaceholder);
        } else if ($targetCategory.length) {
            $targetCategory.addClass('drag-over');
            touchPlaceholder.detach();
            $targetCategory.append(touchPlaceholder);
        } else if ($targetList.length) {
            touchPlaceholder.detach();
            $targetList.append(touchPlaceholder);
        }
    });
    
    // 터치 종료
    currentModal.on('touchend', '.menu-customizer-item, .menu-customizer-list, .menu-customizer-category-items', function(e) {
        // 롱프레스 타이머 취소
        if (longPressTimer) {
        ...
        const menuType = touchDraggedItem.data('menu-type');
        
        // 플레이스홀더 위치에 아이템 이동
        if (touchPlaceholder && touchPlaceholder.parent().length) {
            const $parent = touchPlaceholder.parent();
            
            touchPlaceholder.replaceWith(touchDraggedItem);
            
            // 카테고리 업데이트
            if ($parent.hasClass('menu-customizer-category-items')) {
                const newCategoryId = $parent.data('category-id');
                updateItemCategory(menuType, touchDraggedItem.data('item-id'), newCategoryId);
            } else if ($parent.hasClass('menu-customizer-list')) {
                updateItemCategory(menuType, touchDraggedItem.data('item-id'), null);
            }
        ...
        // 정리
        touchDraggedItem.removeClass('dragging');
        currentModal.find('.drag-over').removeClass('drag-over');
    ...
    // 터치 취소
    currentModal.on('touchcancel', function() {
        if (longPressTimer) {
            clearTimeout(longPressTimer);
            longPressTimer = null;
        }
